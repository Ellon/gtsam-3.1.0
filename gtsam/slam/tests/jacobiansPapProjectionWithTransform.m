%% Compute Jacobians for GTSAM's ParallaxAngleProjectionFactors with a sensor transform

% Warning: This code uses SLAMTB functions
...
syms stx sty stz sex sey sez real
...
% compose camera frame
mainCamFrame = composeFrames(mainFrame,sensorFrame);
assoCamFrame = composeFrames(assoFrame,sensorFrame);
...
mainCamFrame = composeFrames(mainFrame,sensorFrame);
assoCamFrame = composeFrames(assoFrame,sensorFrame);
otheCamFrame = composeFrames(otheFrame,sensorFrame);
...
%% Values used in the test projection (3D point is at origin)
p = deg2rad(0); y = deg2rad(90); par = deg2rad(90);
mtx = -0.10; mty = -6.25; mtz = -1.0; mex = 0; mey = 0; mez = deg2rad(90); 
atx =  6.25; aty = -0.10; atz = -1.0; aex = 0; aey = 0; aez = deg2rad(180); 
otx = -6.25; oty =  0.10; otz = -1.0; oex = 0; oey = 0; oez = 0;
stx =  0.25; sty = -0.10; stz =  1.0; sex = deg2rad(-90); sey = 0; sez = deg2rad(-90);
